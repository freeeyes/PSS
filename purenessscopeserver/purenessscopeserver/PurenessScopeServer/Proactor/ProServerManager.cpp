...
bool CProServerManager::AddListen(const char* pListenIP, uint32 u4Port, uint8 u1IPType)
{
	bool blState = m_ConnectAcceptorManager.CheckIPInfo(pListenIP, u4Port);
	if(true == blState)
	{
		//当前监听已经存在，不可以重复建设
		OUR_DEBUG((LM_INFO, "[CProServerManager::AddListen](%s:%d) is exist.\n", pListenIP, u4Port));
		return false;
	}

	//创建一个新的accept对象
	ProConnectAcceptor* pProConnectAcceptor = m_ConnectAcceptorManager.GetNewConnectAcceptor();
	if(NULL == pProConnectAcceptor)
	{
		OUR_DEBUG((LM_INFO, "[CProServerManager::AddListen](%s:%d) new ConnectAcceptor error.\n", pListenIP, u4Port));
		return false;
	}
	...
	if(nErr != 0)
	{
		OUR_DEBUG((LM_INFO, "[CProServerManager::AddListen](%s:%d)set_address error[%d].\n", pListenIP, u4Port, errno));
		return false;
	}
	...
	if(NULL == pProactor)
	{
		OUR_DEBUG((LM_INFO, "[CProServerManager::AddListen]App_ProactorManager::instance()->GetAce_Proactor(REACTOR_CLIENTDEFINE) is NULL.\n"));
		return false;
	}
	...
	if(-1 == nRet)
	{
		OUR_DEBUG((LM_INFO, "[CProServerManager::AddListen] Listen from [%s:%d] error(%d).\n",listenAddr.get_host_addr(), listenAddr.get_port_number(), errno));
		return false;
	}

	OUR_DEBUG((LM_INFO, "[CProServerManager::AddListen](%s:%d)Add Listen success.\n", pListenIP, u4Port));

	return true;
}

bool CProServerManager::DelListen(const char* pListenIP, uint32 u4Port)
{
	bool blState = m_ConnectAcceptorManager.CheckIPInfo(pListenIP, u4Port);
	if(false == blState)
	{
